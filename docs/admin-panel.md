...
Set `PZ_MOD_REFRESH_WINDOW_START` and `PZ_MOD_REFRESH_WINDOW_END` to control the smart-refresh window.

Set `PZ_AUTO_REFRESH_MODS=true` only after the smart-refresh flow has been tested with your mod pack.
