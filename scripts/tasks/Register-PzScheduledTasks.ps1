...
function New-PzTaskAction {
    ...
    return New-ScheduledTaskAction -Execute $pwsh -Argument $args -WorkingDirectory $config.ProjectRoot
}
...
$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -MultipleInstances IgnoreNew -StartWhenAvailable -WakeToRun -ExecutionTimeLimit (New-TimeSpan -Minutes 0)
...
Register-ScheduledTask -TaskName "$TaskPrefix Watchdog" -Action (New-PzTaskAction $watchdogScript) -Trigger (New-ScheduledTaskTrigger -Once -At $firstRun -RepetitionInterval (New-TimeSpan -Minutes 5) -RepetitionDuration (New-TimeSpan -Days 3650)) -Principal $principal -Settings $settings -Force | Out-Null
